...
def order_detail(chat_id, order):
...
def callback(call):
    ...
    if call.message:
        data = call.data
        if data == 'my_orders':
            my_orders(chat_id=chat_id, users=user)
        elif data == 'menu':
            menu(chat_id=chat_id)
